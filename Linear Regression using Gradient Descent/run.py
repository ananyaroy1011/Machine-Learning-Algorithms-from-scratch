...
def load_auto():
	Auto = pd.read_csv(DATA_PATH + 'Auto.csv', na_values='?', dtype={'ID': str}).dropna().reset_index()
...
class LinearRegressionWithGradientDescent():
	...
	def varying_learning_rate_plot(self,X_train,Y_train):
		plt.xlabel('Epoch')
		plt.ylabel('Cost')

		theta,cost = self.train_linear_model(X_train,Y_train,lr=1,epochs=1000)
		plt.plot(cost,label='lr=1')

		theta,cost = self.train_linear_model(X_train,Y_train,lr=0.1,epochs=1000)
		plt.plot(cost,label='lr=0.1')

		theta,cost = self.train_linear_model(X_train,Y_train,lr=0.01,epochs=1000)
		plt.plot(cost,label='lr=0.01')

		theta,cost = self.train_linear_model(X_train,Y_train,lr=0.001,epochs=1000)
		plt.plot(cost,label='lr=0.001')

		theta,cost = self.train_linear_model(X_train,Y_train,lr=0.0001,epochs=1000)
		plt.plot(cost,label='lr=0.0001')
